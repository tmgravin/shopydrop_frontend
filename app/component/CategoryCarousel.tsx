...
//         const interval = setInterval(() => {
//           if (carouselRef.current) {
//             // Add logic here to change the carousel slide
//           }
//         }, 3000); // Change slide every 3 seconds
...
export function CategoryCarousel() {
  ...
  return (
    <Carousel
      ref={carouselRef}
      plugins={[Autoplay({ delay: 5000 })]} // Autoplay plugin for the carousel
      options={{ loop: true }} // Corrected 'opts' to 'options'
      className="w-full shadow-none gap-0"
    >
      <CarouselContent className="w-full flex gap-2">
        {categories.map((category, index) => (
          ...
          >
            <Card className="bg-transparent border-none shadow-none">
              <CardContent className="flex text-black aspect-square items-center justify-center p-0 shadow-none">
                <Image
                  src={category?.categoryImage || "https://via.placeholder.com/600x400.png?text=Category"} // Use placeholder if image not available
                  width={80}
                  height={80}
                  alt={`Category ${category.categoryName}`}
                  className="h-12 w-12 md:h-16 md:w-16 lg:h-20 lg:w-20 rounded-full"
